...
import { StyleSheet, div, p, TouchableOpacity, AppState } from "react-native";
...
class CountDown extends React.Component {
  ...
  renderDigit = d => {
    ...
    return (
      ...
      >
        <p style={[styles.digitTxt, { fontSize: size }, digitTxtStyle]}>{d}</p>
      </div>
    );
  };
  ...
  renderLabel = label => {
    ...
    if (label) {
      return (
        <p style={[styles.timeTxt, { fontSize: size / 1.8 }, timeLabelStyle]}>
          {label}
        </p>
      );
    }
  ...
  renderDoubleDigits = (label, digits) => {
    const { timeLabelStyle, size } = this.props;
    return (
      <div style={styles.doubleDigitCont}>
        <p style={timeLabelStyle}>{digits + label + " "}</p>
      </div>
    );
  };

  renderDoubleDigitsDetails = (label, digits) => {
    const { timeLabelStyle, size } = this.props;
    return (
      <div style={styles.doubleDigitCont}>
        <p style={timeLabelStyle}>{digits + " " + label + " "}</p>
  ...
  renderCountDown = () => {
    const { timeToShow, timeLabels, showSeparator, fromDetails } = this.props;
    const { until } = this.state;
    ...
    const Component = this.props.onPress ? TouchableOpacity : div;

    return (
      <Component
        style={[styles.timeCont, fromDetails ? styles.detailsDesign : ""]}
        onPress={this.props.onPress}
      >
        {timeToShow.includes("D")
          ? fromDetails
            ? this.renderDoubleDigitsDetails(timeLabels.d, newTime[0])
            : this.renderDoubleDigits(timeLabels.d, newTime[0])
          : null}
        {fromDetails ? <p>|</p> : null}
        ...
        {timeToShow.includes("H")
          ? fromDetails
            ? this.renderDoubleDigitsDetails(timeLabels.h, newTime[1])
        ...
        {timeToShow.includes("M")
          ? fromDetails
            ? this.renderDoubleDigitsDetails(timeLabels.m, newTime[2])
        ...
        {timeToShow.includes("S")
          ? fromDetails
            ? this.renderDoubleDigitsDetails(timeLabels.s, newTime[3])
  ...
  render() {
    return <div style={this.props.style}>{this.renderCountDown()}</div>;
  }
}
